...
![**Hash-based alignment**](images/hash%20table%20alignment.png){width="399"}

**Hash-Based Alignment** is a widely used strategy in early and some modern short-read mappers. It relies on breaking sequencing reads into short, fixed-length substrings called **k-mers** (e.g., a 31-mer is a string of 31 nucleotides), and using a hash table to index either the reference genome or the read-derived k-mer.

During alignment, reads are decomposed into short k-mer "seeds", which serve as initial exact matches used to anchor the read to candidate genomic locations. These candidate regions are then *extended* (hence, **seed-and-extend**) and refined us a local alignment algorithm such as Smith-Waterman, to account for mismatches and small insertions or deletions. The final alignments are typically saved in formats like **BAM** and **CRAM**. Tools may differ in whether they hash the reads or the reference: **MAQ, ELAND, ZOOM, and SHRiMP** hash the reads (offering flexibility but variable memory usage), while **SOAP, BFAST, and MOSAIK** hash the reference, providing more consistent memory usage.

### FM-index / BWT-Based Alignment

**FM-index-Based Alignment** uses a compressed full-text indexing structure derived from the **Burrows-Wheeler Transform (BWT)** to enable fast and memory-efficient searching of sequencing reads against a reference genome. Unlike hash-based approaches, FM-index methods do not explicitly store k-mer lookup tables. Instead, they enable exact or near-exact pattern matching by performing backward search over a compressed representation of all suffixes of the reference genome. This approach significantly reduces memory usage while maintaining high alignment speed, and tools such as **MUMmer**, **BWA**, and **Bowtie** use it to quickly identify candidate mapping locations. However, they still require a **final local alignment step** (e.g., Smith-Waterman) to accurately map reads and account for mismatches and small insertions or deletions.
